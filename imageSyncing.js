...
exports.handler = function(event, context) {
    ...
    console.log("records",event.Records[0].s3.object.key);

    var srcKey = decodeURIComponent(event.Records[0].s3.object.key.replace(
        /\+/g, " "));
	console.log("sourceBucket: ",srcBucket);
    console.log("srcKey: ",srcKey);
    var userId = srcKey.substring(srcKey.indexOf("/")+1,srcKey.lastIndexOf("/"));
    ...
    async.forEachOf(_sizesArray, function(value, key, callback) {
        async.waterfall([
            ...
            function upload(data, index, next) {
                ...
                console.log("upload to path :", "file-upload/" + userId  +
                        "/" + fileName.slice(0, -4) +
                        ".jpg");
